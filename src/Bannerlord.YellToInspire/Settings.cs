...
namespace Bannerlord.YellToInspire
{
    public sealed class Settings : AttributeGlobalSettings<Settings>
    {
        ...
        [SettingPropertyFloatingInteger("{=pVdQUEHQub}Ability Cooldown Decrease per Level", 0f, 200f, "#0 ms", Order = 3, RequireRestart = false, HintText = "{=dmGkKtAq4M}Determines how much less cooldown time in seconds per level in Leadership.")]
        public float AbilityCooldownDecreasePerLevel { get; set; } = 150f;


        [SettingPropertyFloatingInteger("{=xEeTLJBPXY}Base Allied Morale Gain", 0f, 100f, "#0", Order = 4, RequireRestart = false, HintText = "{=lYflwGnkSY}Determines the base anount of how much morale each friendly unit gains per yell, also the value for Custom Battles.")]
        public float BaseAlliedMoraleGain { get; set; } = 0f;

        [SettingPropertyFloatingInteger("{=UIOWxJ8oHq}Allied Morale Gain Increase per Level", 0f, 1f, "0.00", Order = 5, RequireRestart = false, HintText = "{=KFuKPM6vlD}Determines how much extra morale gain per level in Leadership.")]
        ...
        [SettingPropertyFloatingInteger("{=ykC6USItRT}Max Response Delay", 0f, 10f, "0.00 sec", Order = 10, RequireRestart = false, HintText = "{=nAwgC95YKZ}The maximum amount of random time in seconds before a unit responds to your yell.")]
        public float MaxResponseDelay { get; set; } = 2.2f;


        [SettingPropertyFloatingInteger("{=MVuF5MdRAd}Leadership Exp per Ally", 0f, 100f, "#0 exp", Order = 11, RequireRestart = false, HintText = "{=DOR2xn2cYG}The base amount (which is multiplied by attribute and focus bonus) of Leadership exp gained per ally affected by Inspire.")]
        public float LeadershipExpPerAlly { get; set; } = 1f;

        [SettingPropertyFloatingInteger("{=mnVFi42L7r}Roguery Exp per Enemy", 0f, 100f, "#0 exp", Order = 12, RequireRestart = false, HintText = "{=z8P8DzRpwc}The base amount (which is multiplied by attribute and focus bonus) of Roguery exp gained per enemy affected by Inspire.")]
        public float RogueryExpPerEnemy { get; set; } = 1f;


        //[SettingPropertyBool("{=I7z9HhTKKJ}Fled Enemies Return", Order = 13, RequireRestart = false, HintText = "{=n6ViryinCm}Determines if fleeing enemies return to battle, like a taunt of sorts.")]
        ...
        [SettingPropertyBool("{=oIjsBtyRi6}Show Detailed Message Text", Order = 16, RequireRestart = false, HintText = "{=qiJ4Uud7qw}Displays message containing a rundown of the number of friendly and enemy units affected per ability use.")]
        public bool ShowDetailedMessageText { get; set; } = true;


        [SettingPropertyBool("{=MBlehjhwnx}Enable Cheer Random Delay", Order = 17, RequireRestart = false, HintText = "{=CTT0d1Q6Q7}Determines if units individually respond to yells with a small random amount of delay.")]
        public bool EnableCheerRandomDelay { get; set; } = true;

        [SettingPropertyBool("{=GCpn22ub5U}Enable Cheer Animation", Order = 18, RequireRestart = false, HintText = "{=cPUY6zLpa6}Determines if the player and allies units play cheering animations.")]
        public bool EnableCheerAnimation { get; set; } = true;


        [SettingPropertyBool("{=EYQuao3mup}Enable Enemy Reaction", Order = 19, RequireRestart = false, HintText = "{=YncLuM2Sps}Determines whether enemies verbally respond in fear to your yells.")]
